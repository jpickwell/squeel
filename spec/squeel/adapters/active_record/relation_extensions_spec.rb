      ...
      describe RelationExtensions do
        ...
        describe '#build_arel' do
          ...
          it 'handles multiple wheres using a keypath' do
            relation = Person.joins { articles }
                             .where { articles.title == 'Hello' }
          ...
          it 'maintains groupings as given' do
            relation = Person.where(dsl do
              (name == 'Ernie') | ((name =~ 'Bob%') & (name =~ '%by'))
            end)
            arel = relation.build_arel
          ...
          it 'does not inadvertently convert KeyPaths to booleans when uniqing where_values' do
            100.times do # Doesn't happen reliably because of #hash behavior
              persons = Person.joins do
                [outgoing_messages.outer, incoming_messages.outer]
              end

              persons = persons.where do
                (
                  (outgoing_messages.author_id.not_eq 7) &
                  (incoming_messages.author_id.not_eq 7)
                )
              end

              persons = persons.where do
                (
                  (outgoing_messages.recipient_id.not_eq 7) &
                  (incoming_messages.recipient_id.not_eq 7)
                )
              end
          ...
          it 'reverses order of Arel::Attributes when #last is called' do
            sorted_people = Person.all.to_a.sort do |a, b|
        ...
        describe '#to_sql' do
          it 'casts a non-acceptable value for a Function key properly in a hash' do
            relation = Person.joins(:children).where(
              children: {
                :coalesce.func(:name, 'Mr. No-name') => 'Ernie'
              }
            )
          ...
          it 'casts a non-acceptable value for a Predicate containing a Function expr properly' do
            relation = Person.joins(:children).where(
              children: {
                :coalesce.func(:name, 'Mr. No-name').eq => 'Ernie'
              }
            )
            relation.to_sql.should match(/'Ernie'/)
          end
          ...
          it 'casts a non-acceptable value for a KeyPath with a Function endpoint properly' do
            relation = Person.joins(:children).where do
              { children.coalesce(:name, 'Mr. No-name') => 'Ernie' }
            end
            relation.to_sql.should match(/'Ernie'/)
          end
          ...
          it 'casts a non-acceptable value for a KeyPath with a Predicate endpoint containing a Function expr properly' do
            relation = Person.joins(:children).where do
              { children.coalesce(:name, 'Mr. No-name').eq => 'Ernie' }
            end
            relation.to_sql.should match(/'Ernie'/)
          end
        ...
        describe '#includes' do
          it 'builds options with a block' do
            standard = Person.includes(children: :children).where(
              children: {
                children: { name: 'bob' }
              }
            )
            block = Person.includes { { children => children } }.where(
              children: {
                children: { name: 'bob' }
              }
            )
          ...
          it 'eager loads multiple top-level associations with a block' do
            standard = Person.includes(:children, :articles, :comments).where(
              children: { name: 'bob' }
            )
            block = Person.includes { [children, articles, comments] }.where(
              children: { name: 'bob' }
            )
            block.debug_sql.should eq standard.debug_sql
          end
          ...
          it 'eager loads polymorphic belongs_to associations' do
            relation = Note.includes { notable(Article) }
          ...
          it 'only includes once, even if two join types are used' do
            relation = Person.includes(:articles.inner, :articles.outer)
        ...
        describe '#group' do
          ...
          it 'returns size correctly using group' do
            if activerecord_version_at_least('3.2.0')
              ...
              relation.size[Person.first.id].should eq 3
            end
        ...
        describe '#where' do
          ...
          it 'builds compound conditions with a block' do
            block = Person.where { (name == 'bob') & (salary == 100_000) }
          ...
          it 'maps conditions onto their proper table with multiple polymorphic joins' do
            ...
            people_notes = relation.where { notable(Person).salary > 30_000 }
            article_notes = relation.where { notable(Article).title =~ '%' }
            people_and_article_notes = relation.where { (notable(Person).salary > 30_000) | (notable(Article).title =~ '%') }
        ...
        describe '#joins' do
          ...
          it 'joins an ActiveRecord::Relation subquery' do
            subquery = OrderItem.group(:orderable_id)
                                .select { [orderable_id, sum(quantity * unit_price).as(amount)] }

            relation = Seat.joins { [payment.outer, subquery.as('seat_order_items').on { id == seat_order_items.orderable_id}.outer] }
        ...
        describe '#where_unscoping' do
          it "doesn't ruin everything when predicate expression in where_values doesn't respond to :symbol method" do
            ...
            elsif activerecord_version_at_least '4.0.0'
        ...
        describe '#to_a' do
          it 'eager-loads associations with dependent conditions' do
            relation = Person.includes(:comments, :articles).where do
          ...
          it 'includes a belongs_to association even if the child model has no primary key' do
            relation = UnidentifiedObject.where { person_id < 120 }
                                         .includes(:person)
            queries = queries_for do
              vals = relation.to_a
